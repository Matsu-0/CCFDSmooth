...
public class KDTreeUtil {
    ...
    public ArrayList<ArrayList<Double>> queryRecKNN(ArrayList<Double> input, double distance, Stack<Node> stack,
            double[] std) {
    ...
    public Double get_tm_distance(ArrayList<Double> tuple, ArrayList<Double> nearest, double[] std) {
        double dist = UtilZ.distance(tuple, nearest, std);
        return dist;
    }
        ...
        static double mindistance(ArrayList<Double> a, ArrayList<Double> max, ArrayList<Double> min, double[] std) {
            ...
            for (int i = 0; i < a.size(); i++) {
                if (a.get(i) > max.get(i))
                    sum += Math.pow((a.get(i) - max.get(i)) / std[i], 2);
